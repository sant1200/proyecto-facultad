...
export default function Home() {
  ...
  const startExam = async (generateNew: boolean = true) => {
    if (!currentSession || !currentFileBase64) return;
    
    setIsRegeneratingExam(true);
    try {
      const action = generateNew ? 'regenerateExam' : 'exam';
      const body: Record<string, unknown> = {
        action,
        fileBase64: currentFileBase64,
        fileType: currentFileType,
        fileName: currentSession.documentName,
        numQuestions: 10
      };
  ...
  const handleRegenerateFlashcards = async () => {
    if (!currentSession || !currentFileBase64 || isRegeneratingFlashcards) return;
    ...
    try {
      ...
      if (!res.ok) throw new Error('Error regenerando flashcards');
      const result = await res.json();

      const newFlashcards = (result.flashcards || []).map((f: { question: string; answer: string }) => ({
        ...createEmptyFlashcard(),
        id: generateId(),
        question: f.question,
        answer: f.answer
      ...
      const updatedSession = { ...currentSession, flashcards: newFlashcards };
      setCurrentSession(updatedSession);
      updateFlashcards(currentSession.id, newFlashcards);
      setCurrentCardIndex(0);
      setShowAnswer(false);
    } catch (error) {
      console.error('Error regenerating flashcards:', error);
      alert('Error al regenerar flashcards. Intenta de nuevo.');
    } finally {
  ...
  const handleRegenerateQuiz = async () => {
    if (!currentSession || !currentFileBase64 || isRegeneratingQuiz) return;
    ...
    try {
      ...
      if (!res.ok) throw new Error('Error regenerando quiz');
      const result = await res.json();

      const newQuiz = (result.quizQuestions || []).map((q: { question: string; options: string[]; correctAnswer: number; explanation?: string }) => ({
        id: generateId(),
        question: q.question,
        options: q.options,
        correctAnswer: q.correctAnswer,
      ...
      const updatedSession = { ...currentSession, quizQuestions: newQuiz };
      setCurrentSession(updatedSession);
      updateCurrentSession(updatedSession);
      setQuizAnswers({});
      ...
      alert('Error al regenerar quiz. Intenta de nuevo.');
    } finally {
  ...
  const selectSession = (session: StudySession) => {
    ...
    setCurrentFileBase64(session.fileBase64 || '');
    setCurrentFileType(session.fileType || '');
    setMode('study');
    setStudyTab('summary');
  };
                  ...
                  <div className="text-center">
                    <div className="flex justify-between items-center mb-6">
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 bg-purple-500/20 text-purple-400 hover:bg-purple-500/30 rounded-lg text-sm font-medium border border-purple-500/20 transition-all disabled:opacity-50"
                      >
                        {isRegeneratingFlashcards ? '🔄_generando...' : '🔄 Regenerar'}
                      </button>
                    </div>
                ...
                <div className="glass p-6 rounded-2xl border border-white/5">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 bg-purple-500/20 text-purple-400 hover:bg-purple-500/30 rounded-lg text-sm font-medium border border-purple-500/20 transition-all disabled:opacity-50"
                    >
                      {isRegeneratingQuiz ? '🔄_generando...' : '🔄 Regenerar'}
                    </button>
                  </div>
          ...
          <div className="animate-fade-in">
            <div className="glass p-6 rounded-2xl mb-6 border border-white/5">
              <div className="flex justify-between items-center">
                ...
                <button
                  onClick={() => { setIsRegeneratingExam(true); startExam(); }}
                  disabled={isRegeneratingExam || !currentFileBase64}
                  className="px-4 py-2 bg-purple-500/20 text-purple-400 hover:bg-purple-500/30 rounded-lg text-sm font-medium border border-purple-500/20 transition-all disabled:opacity-50"
                >
                  {isRegeneratingExam ? '🔄 Generando...' : '🔄 Nuevas Preguntas'}
                </button>
              </div>
            </div>
